...
export const Carousel = ({width, height, images, placeholder, ...rest}: CarouselProps) => {
  ...
  const scheduledAnimationFrame = useRef<number>(0)
  ...
  const goToIndex = useCallback(
    ({index, direction = 1}: {index: number; direction?: number}) => {
      ...
      if (uniforms.dispFactor.value !== 1) {
        ...
        animate(uniforms.dispFactor.value as number, 1, {
          type: 'spring',
          stiffness: 100,
          damping: 20,
          restSpeed: 0.001,
  ...
  const navigate = useCallback(
    ({direction, index = null, ...rest}: NavParams) => {
      if (!loaded) return
  ...
  const onSwipeEnd = useCallback(() => {
    ...
    const uniforms = material.current.uniforms
    const duration = (1 - (uniforms.dispFactor.value as number)) * 1000
