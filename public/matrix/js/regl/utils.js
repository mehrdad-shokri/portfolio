// PORT PATCH: resolve lib/shader/asset URLs against this vendored folder
// (…/public/matrix/) instead of the host page's document URL.
const RESOURCE_BASE = new URL("../../", import.meta.url).href;
const resolveResource = (url) =>
	url == null || /^(https?:)?\/\//.test(url) || url.startsWith("/") ? url : RESOURCE_BASE + url;
...
const loadText = (url) => {
	...
	return {
		...
		loaded: (async () => {
			if (url != null) {
				text = await (await fetch(resolveResource(url))).text();
				loaded = true;
			}
		})(),
	};
